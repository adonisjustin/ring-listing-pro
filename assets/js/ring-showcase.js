...
$(document).on('click', '.rsp-quick-view', (e) => {
    this.openQuickView(e);
        ...
        collectFilters() {
            ...
            // Checkboxes
            const filterTypes = ['carat', 'color', 'vendor', 'category'];
        ...
        loadRings(append = false) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    orderby: sortBy === 'price' ? 'meta_value_num' : sortBy,
                    order: sortOrder === 'desc' ? 'DESC' : 'ASC',
                    meta_key: sortBy === 'price' ? 'ring_price' : ''
                },
                success: (response) => {
    ...
    if (response.success) {
        ...
        if (append) {
            response.rings.forEach(ring => {
                console.log('Appending ring HTML:', ring.substring(0, 200)); // Debug line
                this.grid.append(ring);
            });
        } else {
            console.log('Setting grid HTML with', response.rings.length, 'rings'); // Debug line
        ...
        // Show/hide load more
        if (this.currentPage >= response.pages || response.total === 0) {
            $('#rsp-load-more').hide();
        } else {
            $('#rsp-load-more').show();
        }
